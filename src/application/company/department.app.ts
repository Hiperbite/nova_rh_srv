...
import { Contract, Department, Employee, Person, Track, User } from "../../models/index";
...
export class DepartmentApp {
  ...
  static filter = (filter: any) => {
    const { q, code, name, roleId, departmentId, date, antiqueness } = filter

    let where: any = {}
    const include: any = Department?.options;
