...
import sequelize, { Country, Role, PayrollStatus, AdditionalPaymentType, Department, Company, Setting } from "./index";
...
const initializer = (_?: any) =>
    initialData.forEach(({ model, data }: any) => data.forEach(async (d: any) => {
        try {
            ...
            sequelize.getRepository(model).findOne({ where: { code: d.code } }).then((f: any) => {
                if (f===null) {
